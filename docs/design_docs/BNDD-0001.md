...
| Related docs | `docs/architecture-decisions.md`, `docs/frontend-architecture.md` |
...
- `docs/` contains architecture and fundamentals notes
...
```mermaid
flowchart TD
    A[Seed Bootstrap\nDNS seeds / known peers] --> B[Frontier Scheduler\nqueue, dedup, retry, backoff]
    B --> C[Connection Worker\nTCP connect + timeout]
    C --> D{Connected?}

    D -- No --> E[Mark Failure\nupdate retry state / confidence]
    E --> B

    D -- Yes --> F[Protocol Handshake\nsend version, read version/verack]
    F --> G{Handshake OK?}

    G -- No --> H[Record Handshake Failure\nreason, endpoint, timestamp]
    H --> B

    G -- Yes --> I[Capture Node Metadata\nservices, user agent, protocol version, height]
    I --> J[Peer Discovery\nsend getaddr]
    J --> K[Parse addr / addrv2]
    K --> L[Normalize Endpoints\ncanonicalize ip/onion + port]
    L --> M[Deduplicate\nalready seen?]
    M --> N[Persist Raw Observation\nobservation history]
    N --> O[Enrichment Layer\nIPv4/IPv6 -> ASN + country\nTor -> network_type only]
    O --> P[Update Current State\nfirst_seen, last_seen, confidence]
    P --> Q[Expand Frontier\nschedule newly discovered peers]
    Q --> B

    P --> R[Analytics Layer\nASN concentration, prefix clustering,\nversion adoption, churn, risk metrics]
```
