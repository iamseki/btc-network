...
#[utoipa::path(
    ...
    description = "Return verified-node protocol-version distribution from the newest run that matches the phase filter. Defaults to the latest finished run.",
    params(
...
pub(crate) async fn list_last_run_protocol_versions(
    ...
    let rows = state
        .analytics_reader
        .list_last_run_protocol_versions(
