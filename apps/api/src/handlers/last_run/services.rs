...
use btc_network::crawler::LastRunServicesCountItem;
...
pub(crate) const LIST_LAST_RUN_SERVICES_PATH: &str = "/api/v1/network/last-run/services";

#[utoipa::path(
    get,
    path = "/api/v1/network/last-run/services",
    summary = "List last-run services distribution",
    description = "Return advertised Bitcoin service-flag distribution from the newest run that matches the phase filter. Defaults to the latest finished run.",
    params(
    ...
    responses(
        (status = 200, description = "Last-run services distribution.", body = RowsResponse<LastRunServicesCountItem>),
